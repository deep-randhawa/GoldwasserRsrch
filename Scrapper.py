...
def get_member_with_url(member_url):
    """
    :param member_url:
    :return:
    """
    if type(member_url) is unicode:
        member_url = str(member_url)
    if type(member_url) is not str:
        print >> sys.stderr, 'member_url should be a str'
        sys.stderr.flush()
        return None
    if not member_url.startswith('/'):
...
def get_debate_with_url(debate_url):
    """
    :param debate_url:
    :return:
    """
    debate = BeautifulSoup(urllib.urlopen(root + debate_url).read(), 'lxml').find('div', id='debate')
    ...
    debate_obj = Debate(title=title, link=debate_url, debate_no=params_dict['Debate No'],
    ...
    for page_num in range(1, num_pages):
        try:
            ...
            for debate_num in debates_on_this_page.find_all('li', class_='debate'):
                ...
                debate_obj = get_debate_with_url(link)
